...
class FeatEncoder(TransformerMixin):
    ...
    def __init__(self, 
        ...
        feats_unary: list
            List of unary features , default []
        feats_null: list
            List of null features (null feature or with more than n threshold), default []
        null_threshold: float
            Threshold to determine if it is null or not (if it exceedes this threshold then its null), default 0.5
        unary_threshold: float
            Threshold to determine if it is unary or not (if it exceedes this threshold then its unary), default 0.9
        bin_transform: bool
        ...
        self.feats_unary = feats_unary 
        self.feats_null = feats_null
        self.__null_threshold = null_threshold
        self.__unary_threshold = unary_threshold
        ...
        self.__all = self.feats_tgt+self.feats_id+self.feats_bin+self.feats_cont+self.feats_ord+self.feats_str+self.feats_date+self.feats_other+self.feats_cluster+self.feats_unary
    ...
    def fit(self, df):
        ...
        self.feats_null = self.feats_null + self.get_null_vars(work_df[feats],self.__null_threshold)
        feats = list(set(feats)-set(self.feats_null))
        ...
        self.names.update({col:f'unary_{self.clean_names(col,symbols_nothing=self.__symbols_nothing, symbols_underscore=self.__symbols_underscore, method=self.__method)}' if not col.startswith('unary_') else col for col  in self.feats_unary})
        self.names.update({col:f'null_{self.clean_names(col,symbols_nothing=self.__symbols_nothing, symbols_underscore=self.__symbols_underscore, method=self.__method)}' if not col.startswith('null_') else col for col  in self.feats_null})
        ...
        self.feats_null = [col for col in self.names.values() if col.startswith('null_')]
        self.feats_unary = [col for col in self.names.values() if col.startswith('unary_')]
    ...
    def update_lists(self, df):
        ...
        self.feats_null = [col for col in  work_df.columns if col.startswith('null_')]
        self.feats_unary = [col for col in  work_df.columns if col.startswith('unary_')]
        self.feats_numeric = self.feats_bin+self.feats_cont+self.feats_ord
    ...
    @staticmethod
    def get_null_vars(work_df, threshold=0.5):
        nulls = (work_df.isnull().sum()/work_df.shape[0])
    
        return list(nulls[nulls>=threshold].index)
